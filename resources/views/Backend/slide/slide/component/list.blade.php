...
<div id="sortable" class="row slide-list sortui ui-sortable">
    ...
    @if(!empty($slides))
        @foreach($slides as $parentId => $slide)
            @php
                $tab_1 = "tab_" . $counter;
                    ...
                    <div class="row custom-row">
                        <div class="col-lg-3">
                            <span class="slide-image img-cover">
                                ...
                                <input type="hidden" name="slide[id][]" value="{{ $parentId }}">
                                    ...
                                    <div id="{{ $tab_1 }}" class="tab-pane active">
                                        <div class="panel-body">
                                            ...
                                            <div class="form-row mb10">
                                                <textarea name="slide[description][]" class="form-control">{{ $slide['description'] }}</textarea>
                                            </div>
                                            <div class="form-row form-row-canonical">
                                                <input type="text" name="slide[canonical][]" class="form-control" placeholder="URL" value="{{ $slide['canonical'] }}">
                                                <div class="overlay">
                                                    <div class="uk-flex uk-flex-middle">
                                                        <label for="input_{{ $tab_1 }}">Mở trong tab mới</label>
                                                        <input type="checkbox" name="slide[window][]" value="_blank" id="input_{{ $tab_1 }}" {{ $slide['window'] == '_blank' ? 'checked' : '' }}>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                    ...
                                    <div id="{{ $tab_2 }}" class="tab-pane">
                                        <div class="panel-body">
                                            ...
                                            <div class="form-row form-row-canonical slide-seo-tab">
                                                <input type="text" name="slide[name][]" class="form-control" placeholder="Tiêu đề ảnh" value="{{ $slide['name'] }}">
                                            </div>
                                            <div class="label-text mt12">Mô tả ảnh:</div>
                                            <div class="form-row form-row-canonical slide-seo-tab">
                                                <input type="text" name="slide[alt][]" class="form-control" placeholder="Mô tả ảnh" value="{{ $slide['alt'] }}">
                                            </div>
                                        </div>
                                    </div>
                                </div>
